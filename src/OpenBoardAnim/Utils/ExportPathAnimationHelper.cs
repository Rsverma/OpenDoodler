...
{
    // Export's own copy of the stroke-by-stroke path-draw animation, split from
    // PathAnimationHelper (which now belongs solely to preview) so export's timing model - real-
    // time Storyboards today, a deterministic per-frame clock later - can change independently of
    // what preview does. Keep this and PathAnimationHelper in sync only when a fix genuinely
    // applies to both; otherwise let them diverge on purpose.
    public class ExportPathAnimationHelper
    {
        ...
        public ExportPathAnimationHelper(Canvas canvas,
                                   List<Path> paths,
